...
%
% OUTPUT (in degrees):
%    dist = arc-distances from (slat, slon) to each (rlat, rlon)
%
...
dist = deg * acos( sin(lar).*sin(las) + cos(lar).*cos(las).*cos(lod) );

% Here is the formula when the points are expressed as th, phi
% dist = acos( cos(th1)*cos(th2) + sin(th1).*sin(th2).*cos(ph1 - ph2) );
