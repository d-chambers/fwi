...
The matlab code was also converted to a small python package (called raytape) so it 
can be run without a matlab license. The source code and the report files are 
...
and, if we rewrite the previous expressions in vector notation, it follows that:

$$
\delta T_{i} = G_{ik} \delta m_k  
$$
...
What is the value of G_{ik} with i = 126 and k = 204? What does each row of G
...
Each row of G (`i` index) corresponds to a particular ray and each column (`j` index)
corresponds to the value of a spline basis function.
...
Compute δm for a range of $\lambda$ values: for example, try
lamvec = 10.^linspace( log10(minlam), log10(maxlam), numlam);
with $\lambda$ min = 0.1 and λ max = 40.0.
...
some value of $\lambda$ is required, but it is somewhat subjective to determine the right
value if the initial model perturbations weren't as obvious.

Using the same parameters as before, the inversion was run again but this time only
including 5 events. The results are similar to those produced by using inversion with
all events but does lack some resolution. 
