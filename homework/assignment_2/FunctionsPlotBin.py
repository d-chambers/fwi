...
def get_continuous_cmap(hex_list, float_list=None):
    ...
    If float_list is not provided, colour map graduates linearly between
    each color in hex_list.
    If float_list is provided, each color in hex_list is mapped to the
...
def read_fortran(filename):
    ...
    with open(filename, "rb") as f:
        # read size of record
        f.seek(0)
        _ = np.fromfile(f, dtype="int32", count=1)[0]
...
def plotbin(x_coords_file, z_coords_file, database_file, figname, figlabel):
    ...
    vmin = np.min(v) - 0.1 * np.max(v)
    vmax = np.max(v) + 0.1 * np.max(v)
