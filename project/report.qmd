...
Specster provides a few different abstraction levels for performing full waveform inversion. Using `Control2d` directly is the lowest level of abstraction, and provides the most control over the inersion process.
...
6. Conduct a line search to find the optical step size, with a maximum update of ~2% (by default). The [golden section search](https://en.wikipedia.org/wiki/Golden-section_search) is used for this.
