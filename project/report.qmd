...
Forward simulations are run with the `Control2d.run` method. You will see the Specfem2d output piped to the terminal or notebook upon calling this method. The output is also written to the output directory so it can be checked later. 
...
Stations and sources can be modified directly through the `station` and `sources` parameter, making it much more repeatable to modify the experiment geometry than manually editing text files. Specster also provides some validation in this aspect to help reduce errors. Here are a few examples:
...
Of course, you can just modify the par_file directly in a text editor then create a new `Control2d` object to update the state.
...
Specster provides a few different abstraction levels for performing full waveform inversion. Using `Control2d` directly is the lowest level of abstraction, and provides the most control over the inversion process.
...
Next, the misfit classes from the `fwi` module are used to calculate the misfit and adjoint sources. We will simply use the waveform misfit but travel time and amplitude misfit functions are also implemented.
...
Now we load the kernel and plot.
...
Before moving onto multiple stations/events, let's take a closer look at the misfit class and how to extend it. Here is the definition of the waveform misfit:
...
You can also control how the preprocessing is conducted in a few ways. By default, the traces are detrended and tapered. The taper percentage is controlled by the `taper_precentage` attribute which is 0.95 (95%) by default. `normalize_traces` controls whether each trace is normalized to its maximum amplitude during preprocessing. For even more control, the `preprocess_traces` method can be implemented in which case the subclass will be in complete control of preprocessing.
...
Waveforms can also be windowed by creating a windowing dataframe, which will tell the Misfit class which parts of the waveform to use and zero the rest in the adjoint output.


## Multiple Events and Multiple Stations

For the full FWI worklow needs to account for multiple events and stations. One simple case is to use a nearly homogeneous velocity model with an inclusion in the center and very good coverage of events and stations. Specster's "inclusion_2d" dataset was created for this purpose.
...
This will create a directory called "EACH_SOURCE" and populate it with copies of the specfem DATA directories although each has been modified to only include a single source. The source directories are simply numbered sequentially starting with "000000".
...
It will then get tedious to work with multiple `Control2d` objects for each source, but Specster provides a higher level control class for facilitating the FWI workflow called `Inverter`.
...
`Inverter` is extensible through both initialization parameters (see its docs) and also inheritance, but here is the basic workflow conducted with `run_iteration`:

1. Ensure each source has its own specfem files
2. Run the forward problem for each source
3. Calculate misfit and adjoint sources for each source then save adjoints to each source data directory
4. Run all sources in adjoint mode
5. Sum the gradients for each source after preconditioning with the approximated Hessian provided by Specfem2D and applying a median filter to the station locations in the gradient (can be disabled based on `hessian_preconditioning` parameter)
6. Conduct a line search to find the optical step size, with a maximum update of ~2% (by default). Specster uses a simple [golden section search](https://en.wikipedia.org/wiki/Golden-section_search) 
...
At each step the inversion state is checkpointed, including misfit, model misfit, and step size. That way the inversion can be restarted from any point.
...
Stopping criteria can be manually specified and the loop exited when those are met. An existing inverter can be loaded via `load_inverter`:
...
Using the data stored in the inverter, we can also plot the convergence as a function of iteration for data misfit, and, since we know the true model in this case, model misfit (L2 norm of the difference between VS models). 

![Model convergence](outputs/a064_fwi_workpath/mode_convergence.png)

A few things to note about this example. First, although the outline of the inclusion is visible, there is still quite a bit of smearing. Also note that the station imprints are not completely removed as there are dashes of lower velocity outside the inclusion. A few options exist that might improve the results. First, rather than waveform misfit, travel_time might be more robust. Also, there was no smoothing applied here, but if the `smoothing_sigma` parameter is defined when creating an `Inverter` instance, it may help remove the artifacts outside the inclusion.

# Conclusions

Creating Specster helped me to better understand a variety of topics including Specfem2d and the general FWI workflow. Although it still lacks some major features, like L-BFGS for model updates and various misfit functions, it may be a useful tool for future FWI research.

# Appendix A: FWI of an Acoustic Reflector

This appendix provides an example of using specster to detect an acoustic reflector, similar to an RTM type workflow. In this case we will only run one iteration and use the kernel as an "image" which will help highlight the location of the reflector.

## Setup true and initial models

First, the ture and initial model are created.
...
![](outputs/a110_acoustic_ref_true/geometry.png)
...
![](outputs/a110_acoustic_ref_initial/geometry.png)
...
Plotting the resulting c kernel we see the reflector interface is illuminated for the center of the model. However, there isn't sufficient data coverage to illuminate the reflector interface near the edges. An imprint of the source frequency is also visible, somewhat "smearing" the reflectors surface. However, the image still provides useful information about the surface of the third layer.

![](outputs/a112_acoustic_ref_fwi/model_updates.png)
